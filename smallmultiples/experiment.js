(function() {
  ...
  var years = 15;//DelayModesData.NONE.length;
  var yearStart = 1999;
  var trackIdx = 2;
  var numBars = 5;
  var ixns = {};
  ...
  function generateInitialState() {
    ...
    if (debug) {
      ...
      correspondence = CorrespondenceModes.SHOWALL;
 ...
 */
  ...
  function drawChart(data, idx, month_svg) {
  ...
  function renderDataSelection(data, idx, delay) {
    ...
    function checkSticky(id, idx) {
      ...
      var renderedLog = state.eventLog.filter(function(e) { return e.event === 'render'; });
      var lastRendered = renderedLog.slice(-1).pop();
    ...
    function checkSerial(id, idx) {
      // serialize output + sticky render
      // we check if this is the first element in the render queue, and if
      // so we pop it and render, otherwise push it on if there are others
      // in the queue. when deciding to push we might render if this should
      // be the first request in the queue and enough time has passed
      // if there is nothing in the queue, we still need to check if the
      // last render happened beyond the persist timeframe, if it didn't
      // then we push this to the render queue, otherwise render immediately
      var renderedLog = state.eventLog.filter(function(e) { return e.event === 'render'; });
      ...
      if (renderQueue.length > 0) {
        if (renderQueue[0].id === id) {
          // came back after previous id rendered
          renderQueue.shift();
          return true;
        }
        for (var i = 0; i < renderQueue.length; i++) {
          if (renderQueue[i].id > id) {
            if (i === 0 && id === lastRendered.id + 1 && timeDiff >= persist) {
              // first in priority queue, and can render immediately
              return true;
            }
            // found spot to insert into priority queue
            renderQueue.splice(i, 0, {id: id, idx: idx});
            return false;
          }
        }
      }
      if (id !== lastRendered.id + 1 || timeDiff < persist) {
        renderQueue.push({id: id, idx: idx});
        return false;
      }
      return true;
    ...
    function render(id, idx) {
      ...
      if (blocking === BlockingModes.SERIAL && id !== 0) {
        if (!checkSerial(id, idx)) {
          state.eventLog.push({event: 'delayedRender', id: id, dataIdx: idx, ts: Date.now()});
          return;
        }
      }
      ...
      if (blocking === BlockingModes.STICKY || blocking === BlockingModes.SERIAL) {
        setTimeout(function() {
          if (renderQueue.length > 0) {
            ...
            if (blocking === BlockingModes.SERIAL && next.id !== id + 1) {
              return;
            }
    ...
    d3.json('us.json', function(error, us) {
        ...
        svg.selectAll('.states')
            ...
            .on('mouseover', function(d){
              ...
              var currState = state.currentMapPos;
              ...
              if (name != 'MN') {
                ...
                if (widgetMode === WidgetModes.INPLACEMAP) {
                  ...
                  d3.select('#' + currState).classed('states-selected', false);
  ...
  function updateViz(data, step, delay) {
    ...
    if (widgetMode === WidgetModes.SLIDER) {
      ...
      // some closure stuff
      function genClickCallback(data, idx, expDelay) {
        return function() {
          var currIdx = state.currentMapPos;
          state.currentMapPos = idx;
          if (currIdx === idx) {
            return;
          }
      ...
      for (var i = 0; i < years; i++) {
        var b = '<li class="button-widgets" id="button_'+ i +'">'+statesIndex[i]+'</li>';
        $('#buttons-wrapper').append(b);
        $('#button_'+i).hover(genClickCallback(data, i, expDelay));
      }
      if (correspondence === CorrespondenceModes.SHOWALL) {
        state.currentMapPos = -11;
      }
